    ...
    <div id="scanner" class="o-container o-section u-margin-bottom-10">
      ...
      <div class="o-grid u-margin-bottom-4">
        <div class="o-grid__col u-12 u-6@sm">
          <message type="info" icon
            ><div
              v-html="
                'Abrimos nuestra tecnología al mundo para que puedas escanear y etiquetar cualquier tipo de texto de la misma manera que TiPi Ciudadano lo hace con la actividad parlamentaria española. Ya seas periodista, estudiante o te dediques a la investigación, ponemos a tu disposición nuestra base de conocimiento, compuesta por más de 3.000 términos relacionados con las 22 temáticas a las que TiPi da seguimiento.'
              "
            ></div
          ></message>
        </div>
        ...
        <div class="o-grid__col u-12 u-6@sm">
          <p>
            <textarea
              placeholder="Inserta aqui el texto que quieres escanear..."
              v-model="inputText"
              rows="9"
            />
          </p>
      ...
      <div id="result" class="o-section o-grid">
        <div v-if="inProgress || errors" class="o-grid__col u-12">
          <message v-if="errors" type="error" icon
            >Has sobrepasado el límite de escaneos por hora. Vuelve a intentarlo
            pasado un tiempo</message
          >
          <loader
            v-if="inProgress"
            title="Escaneando resultados"
            :subtitle="subtitle"
          />
        </div>
        <div class="o-grid__col u-12 result" v-if="result">
          <h4>Resultado del escáner:</h4>
          <message v-if="!result.topics.length" type="error" icon
          ...
          <div class="o-grid" v-else>
            <div class="o-grid__col u-12 u-7@sm">
              <message type="info" icon
                >Si haces clic en cualquiera de las etiquetas relacionadas con
                tu texto podrás conocer además toda la actividad parlamentaria
                asociada con dicha etiqueta.</message
              >
              <topics-section
                meta="Temáticas tratadas"
            ...
            <div class="o-grid__col u-12 u-5@sm">
              <message type="info" icon
                >Aquí tienes una relación visual de tu texto, para que de un
                primer vistazo veas conexiones interesantes.</message
              >
              <span class="u-text-tbody2"
                >Relación de este texto con nuestras temáticas
                <sup
                  title="El gráfico muestra las temáticas relacionados con el texto y el grado de relación con cada una de ellas."
                  >?</sup
                ></span
              >
            </div>
            <div
              class="o-grid__col u-12 u-text-center u-margin-top-4 u-padding-top-4 u-border-top"
            >
              <csv-download
  ...
  computed: {
    subtitle() {
      return this.estimatedTime
        ? `Tardaremos unos ${this.estimatedTime} segundos en mostrarte resultados. No te vayas`
        : "Ten paciencia, estamos trabajando duro";
    },
    hasInput() {
      ...
      api
        ...
        .catch((error) => {
          if (error.response.status == 429)
            this.errors =
              "Has sobrepasado el límite de escaneos por hora. Vuelve a intentarlo pasado un tiempo.";
          else if (error.response.status == 413)
            this.errors =
              "Fichero demasiado pesado para procesarlo. Pruebe con otro más liviano.";
